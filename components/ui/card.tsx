...
const Card = React.forwardRef<
    HTMLDivElement,
    React.HTMLAttributes<HTMLDivElement> & { variant?: "default" | "elevated" | "bordered" }
>(({ className, variant = "default", ...props }, ref) => (
    <div
        ref={ref}
        className={cn(
            "rounded-card bg-surface-2 text-text-primary outline-none",
            variant === "default" && "shadow-elevated",
            variant === "elevated" && "shadow-elevated",
            variant === "bordered" && "border border-border-default shadow-sm",
            className
        )}
        {...props}
    />
